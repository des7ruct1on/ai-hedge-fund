...
class SimpleGraph(StateGraph):
        ...
        try:
            ...
            if category == "analysis":
                return Command(
                    ...
                    update={"stage": StageEnum.DISCUSSION_NODE},
                )
            elif category == "fact":
                return Command(
                    goto=StageEnum.FACT_NODE,
                    update={"stage": StageEnum.FACT_NODE},
                )
            elif category == "backtest":
                return Command(
                    goto=StageEnum.BACKTEST_NODE,
                    update={"stage": StageEnum.BACKTEST_NODE},
                )
            else:
                return Command(
                    goto=StageEnum.OTHER_NODE,
                    update={"stage": StageEnum.OTHER_NODE},
                )
    ...
    def discussion_node(self, state: State) -> State:
        ...
        try:
            ...
            agent_opinions = self.agent_room.discuss_portfolio(
                state["user_data"], state["news_data"], logger=logger
            )
            ...
            logging.info(
                f"✅ Получено {len(agent_opinions)} мнений от агентов"
            )
            logging.info(
                f"📊 Агрегировано {len(aggregated_decisions)} решений"
            )
    ...
    def backtest_node(self, state: State) -> State:
        try:
            ...
            user_portfolio = state.get("user_data", {})
            news_data = state.get("news_data", {})
            user_input = state.get("message_from_user", "")
            # Определяем количество дней бэктеста из контекста пользователя (по умолчанию 7)
            days = self._infer_backtest_days(user_input)
            if not isinstance(days, int) or days <= 0 or days > 365:
                days = 7

            logger.info("Запускаем бэктест")
            result = self.backtest_engine.run_backtest(
                days, user_portfolio, news_data, logger
            )
            ...
            return Command(
                goto=END,
                update={"message_to_user": markdown_result, "stage": END},
            )
        ...
        try:
            ...
            for decision in aggregated_decisions:
                ...
                for opinion in decision.agent_opinions:
                    context += (
                        f"- {opinion.agent_name}: {opinion.action} "
                        f"(уверенность: {opinion.confidence})\n"
                    )
                ...
                try:
                    ...
                    logging.error(
                        f"Ошибка оценки риска для {decision.ticker}: {e}"
                    )
                    risk_assessments.append(
                        RiskAssessment(
                            ticker=decision.ticker,
            ...
            logging.info(
                f"✅ Оценены риски для {len(risk_assessments)} позиций"
            )

            for risk in risk_assessments:
                logging.info(
                    f"⚠️ {risk.ticker}: уровень риска {risk.risk_level}/10"
                )
    ...
    def _extract_risk_level(self, response: str) -> int:
        ...
        risk_match = re.search(
            r"риск[а-я]*\s*[:\-]?\s*(\d+)", response, re.IGNORECASE
        )
        if risk_match:
    ...
    def _extract_risk_factors(self, response: str) -> list:
        ...
        for line in lines:
            if any(
                ...
                for keyword in ["риск", "опасность", "угроза"]
            ):
    ...
    def _build_finalizer_context(self, state: State) -> str:
        ...
        if risk_assessments:
            ...
            for risk in risk_assessments:
                context += (
                    f"- {risk.ticker}: уровень риска {risk.risk_level}/10\n"
                )
            context += "\n"
    ...
    def _format_backtest_results(self, result) -> str:
        ...
        markdown += (
            f"**Начальная стоимость портфеля:** "
            f"{result.initial_portfolio_value:,.2f} ₽\n\n"
        )
        markdown += (
            f"**Финальная стоимость портфеля:** "
        ...
        if result.ticker_performance:
            ...
            for ticker, perf in result.ticker_performance.items():
                markdown += (
                    f"| {ticker} | {perf['total_pnl']:,.2f} | "
                    f"{perf['total_return_pct']:+.2f} | "
                    f"{perf['avg_confidence']:.1f}/10 |\n"
                )
        ...
        if result.daily_results:
            ...
            for day in result.daily_results:
                ...
                markdown += (
                    f"| {day.date.strftime('%d.%m.%Y')} | {day.ticker} | "
                    f"{day.open_price:.2f} | {day.close_price:.2f} | "
                    f"{price_change_pct:+.2f}% | {signal_emoji} {day.signal} | "
                    f"{day.confidence:.1f}/10 | {day.daily_pnl:,.2f} |\n"
                )
